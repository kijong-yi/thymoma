...
# svglite::htmlSVG(
#   width=925/254,height = 900/254,pointsize = 12*0.7,bg="grey",
#   {
cairo_pdf("figures/vaf_wes_rna_gtf2i.pdf",width=925/254,height = 900/254,pointsize = 12*0.7)

structure(meta_dt$GTF2I_WES_VAF,names=meta_dt$id)[meta_dt$GTF2I_status2=="m"] %>% sort
...
par(mfrow=c(1,1),oma=c(0,0,0,0),mar=c(4,3,3,1), pty="s")
...
points(log10(meta_dt$GTF2I_WES_VAF+1),
       ...
       cex=ifelse(meta_dt$is.rescued, 1.7,1.5),
       lwd=ifelse(meta_dt$is.rescued, 1.5,1))

legend("bottomright",legend = c("Thymic carcinoma",expression("GTF2I"^mut),expression("GTF2I"^WT),"Rescued (n=16)"),
       # pch=21,
       pch=c(23,21,21,21),
...
points(log10(meta_dt$GTF2I_WES_VAF+1)[meta_dt$id == "TCGA-XM-A8RE"],
       log10(meta_dt$GTF2I_RNAseq_VAF+1)[meta_dt$id == "TCGA-XM-A8RE"],
       pch = 21,
       bg = purity.pal(meta_dt$final_cellularity),
       cex=3,
       lwd=ifelse(meta_dt$is.rescued, 1.5,1))
...
cairo_pdf("figures/vaf_example_pie.pdf",width=925/254,height = 400/254,pointsize = 12*0.7)

par(mfrow=c(1,2),mar=c(1,1,1,1))
pie(c("(1.4%)"=0.014," "=1-0.014),col=c("#E72119","#B9B9B9"),init.angle = 90,clockwise = T)
pie(c("(11.3%)"=0.1134," "=1-0.1134),col=c("#E72119","#B9B9B9"),init.angle = 90,clockwise = T)
dev.off()

structure(meta_dt$GTF2I_WES_VAF,names=meta_dt$id)[meta_dt$GTF2I_status2=="m"] %>% sort

meta_dt$GTF2I_WES_VAF[meta_dt$id=="TCGA-XM-A8RE"]
meta_dt$GTF2I_RNAseq_VAF[meta_dt$id=="TCGA-XM-A8RE"]
