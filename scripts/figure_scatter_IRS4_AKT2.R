...
"
  _____                  _           _           _   _   _    _ 
 |  __ \                (_)         | |         | | | | | |  | |
 | |  | | ___ _ __  _ __ _  ___ __ _| |_ ___  __| | | | | |  | |
 | |  | |/ _ \ '_ \| '__| |/ __/ _` | __/ _ \/ _` | | | | |  | |
 | |__| |  __/ |_) | |  | | (_| (_| | ||  __/ (_| | |_| |_|  |_|
 |_____/ \___| .__/|_|  |_|\___\__,_|\__\___|\__,_| (_) (_)  (_)
             | |                                                
             |_|                                                
Depricated!!! see 'figure_scatter_IRS4_AKT.R'
"
...
WT_l10_exp_mx <- l10_exp_mx[,WT_ids]
require(GSVA)
# c2cp_dt <- read_tsv('~sypark/00_Project/01_thymoma/10_Final_data/04_GSVA/genesets/c2.cp.v6.2.symbols.gmt', col_names = F)
# c2cp_list <- c2cp_dt %>% as.data.frame() %>% column_to_rownames('X1') %>% dplyr::select(-X2) %>% t %>% as.data.frame() %>% as.list
...
selected_list <- all_list[c("GO_INSULIN_LIKE_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY","REACTOME_PI3K_CASCADE")]

selected_gs_res <- gsva(WT_l10_exp_mx,selected_list)
all_gs_res <- gsva(WT_l10_exp_mx,all_list)

all_gs_res[1:2,1:2]
meta_dt[1:2,1:2]

l10_exp_mx[1:2,1:2]
plot(WT_l10_exp_mx["IRS4",],selected_gs_res[2,])



# gsva and correlation -------------------------------------------------------------------------------------------------
meta_dt$GTF2I_status2 %>% table()
cor_gexp_with_gsva <- cor(t(all_gs_res),t(WT_l10_exp_mx[c("STAR","IRS4","GPR87","C4orf50","ATP10B","NEFL"),]))
cor_gexp_with_gsva[1:6,1:6]
...
tmp_df <- data.frame(IRS4=pmax(0,WT_l10_exp_mx["IRS4",]),
                     IGFsig=selected_gs_res["GO_INSULIN_LIKE_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY",],
                     PI3K=selected_gs_res["REACTOME_PI3K_CASCADE",],
                     histol=histo_pal[meta_dt$histologic_type[match(colnames(selected_gs_res),meta_dt$id)]],
                     stringsAsFactors=F)
...
mtext("IGF receptor signaling score",side=2,line=2.45)
...
polygon(c(sort(tmp_df$IRS4),rev(sort(tmp_df$IRS4))),c(CI$lwr[order(tmp_df$IRS4)],rev(CI$upr[order(tmp_df$IRS4)])),
        col="#00000010",lty = 0)
points(tmp_df$IRS4,tmp_df$IGFsig,bg=tmp_df$histol,pch=21,cex=1.5)
...
plot(1000,xlim=c(0,max(tmp_df$IRS4)),ylim=c(min(tmp_df$PI3K),max(tmp_df$PI3K)),
     xlab=expression(log[10]~TPM~of~IRS4),ylab="",las=2,xaxt="n")
axis(1,at = c(0,1,2,log10(400)),labels = c(0,10,100,400))
mtext("PI3K cascade score",side=2,line=2.45)
...
polygon(c(sort(tmp_df$IRS4),rev(sort(tmp_df$IRS4))),c(CI2$lwr[order(tmp_df$IRS4)],rev(CI2$upr[order(tmp_df$IRS4)])),
        col="#00000010",lty = 0)
points(tmp_df$IRS4,tmp_df$PI3K,bg=tmp_df$histol,pch=21,cex=1.5)
